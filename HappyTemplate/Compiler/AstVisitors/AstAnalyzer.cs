...
namespace HappyTemplate.Compiler.AstVisitors
{
	class AstAnalyzer : ScopedAstVisitorBase, IGlobalScopeHelper
	{
		...
		public override void AfterVisit(Module node)
		{
			...
			_expressionStack.Push(Expression.Lambda(typeof(Action<HappyRuntimeContext>), MaybeBlock(body), new[] { _runtimeContextExp }));
		}

		public override void Visit(VerbatimSection node)
		{
			base.Visit(node);
			_expressionStack.Push(WriteVerbatimToTopWriter(node.Text));
		}
		...
		public override void AfterVisit(StatementBlock node)
		{
			...
			if (node.GetAnalyzeSymbolsExternally())
				_expressionStack.Push(MaybeBlock(expressions));
			else
				_expressionStack.Push(MaybeBlock(node.SymbolTable.GetParameterExpressions(), expressions));
			base.AfterVisit(node);
		}
		...
		public override void AfterVisit(DefStatement node)
		{
			...
			_expressionStack.Push(initializers.Count == 0 ? Expression.Empty() : (Expression)MaybeBlock(initializers));
			base.AfterVisit(node);
		}
		...
		public override void AfterVisit(OutputStatement node)
		{
			...
			_expressionStack.Push(MaybeBlock(writeExps));
			base.AfterVisit(node);
		}
		...
		public override void AfterVisit(WhileStatement node)
		{
			...
			_expressionStack.Push(Expression.Loop(MaybeBlock(loopExprs), loopContext.BreakLabel, loopContext.ContinueLabel));
		...
		public override void AfterVisit(ForStatement node)
		{
			...
			var loopBody = _expressionStack.PopCast<Expression>();
			...
			_expressionStack.Push(MaybeBlock(parameters, loop));
			base.AfterVisit(node);
		}
		...
		public override void AfterVisit(SwitchStatement node)
		{
			...
			if (node.Cases.Length == 0)
			{
				...
				notSwitchExprs.Add(_expressionStack.Pop());
				_expressionStack.Push(MaybeBlock(notSwitchExprs));
				return;
			}
		...
		{
			base.Visit(node);
			_expressionStack.Push(Expression.Constant(node.Value, node.Value.GetType()));
		}
		...
		public override void AfterVisit(AnonymousTemplate node)
		{
			...
			_expressionStack.Push(MaybeBlock(node.Body.SymbolTable.GetParameterExpressions(), body));
			base.AfterVisit(node);
		}
