...
namespace HappyTemplate.Compiler
{
	public class Lexer
	{
		...
		Stack<LexerState> _stateStack = new Stack<LexerState>();
		...
		//        else
		//            _errorCollector.UnexpectedEndOfInput(_reader.GetHappySourceLocation());
		...
		Token ExtractOtherStatementToken()
		{
			...
			case '!':
				...
				if (_reader.Peek(0) == '=')
				{
					...
					retval = new Token(_reader.GetHappySourceLocation(), HappyTokenKind.OperatorNot, "!");
				break;
			case '&':
				_reader.Read();
				if(_reader.Peek() == '&')
		...
		{
			bool eatMore = true;
			SourceLocation startLocation = _reader.CurrentLocation;
